...
def run_MonteCarlo(event, coefVar=0.1, outerSampling=10000, blockSize=10,
                   seed=1234, verbose=False, failure_domain=None):
	...
	if type(event) is list:
		...
		if failure_domain == 'union':
			def function_union(X):
				...
				for i in range(n_event):
					if event[i].getOperator().getImplementation().getClassName() == 'Less' or \
					   event[i].getOperator().getImplementation().getClassName() == 'LessOrEqual':
						sample[:, i] = sample[:, i] < event[i].getThreshold()
					if event[i].getOperator().getImplementation().getClassName() == 'Greater' or \
					   event[i].getOperator().getImplementation().getClassName() == 'GreaterOrEqual':
						sample[:, i] = sample[:, i] >= event[i].getThreshold()
				return np.atleast_2d(sample.sum(axis=1)).T

			model = ot.PythonFunction(event[0].getFunction().getInputDimension(),
									  event[0].getFunction().getOutputDimension(),
									  func_sample=function_union)
			output = ot.RandomVector(model, antecedent)
			event = ot.Event(output, ot.Greater(), 0.)
			...
			output = ot.RandomVector(model, antecedent)
			new_event = ot.Event(output, ot.Greater(), 0.)
	else:
	...
	dfResult = dfResult.append(pd.DataFrame([result.getProbabilityEstimate()], index=["Probability of failure"]))
	dfResult = dfResult.append(pd.DataFrame([result.getCoefficientOfVariation()], index=["Coefficient of varation"]))
	dfResult = dfResult.append(pd.DataFrame([result.getConfidenceLength()], index=["95 % Confidence length"]))
	dfResult = dfResult.append(pd.DataFrame([result.getOuterSampling()*result.getBlockSize()], index=["Number of calls"]))
...
def run_ImportanceSampling(event, pstar, sd=1., coefVar=0.05, outerSampling=1000,
                           blockSize=10, seed=1234, verbose=False, failure_domain=None):
	"""
	...
	simulation = ot.ProbabilitySimulationAlgorithm(ot.StandardEvent(new_event), experiment)
	simulation.setMaximumOuterSampling(outerSampling)
	simulation.setBlockSize(blockSize)
	simulation.setMaximumCoefficientOfVariation(coefVar)
...
def run_LHS(event, coefVar=0.01, outerSampling=1000, seed=1234, verbose=False, failure_domain=None):
	...
	#~ LHS_opt = ot.SimulatedAnnealingLHS(LHS, SA_profile, ot.SpaceFillingC2())
	#~ LHS_opt.generate()
...
def run_SubSet(event, coefVar=0.05, outerSampling=1000, seed=1234, verbose=False, failure_domain=None):
	...
	#Run LHS simulation
	simulation = ot.SubsetSampling(event, 2., 0.05)
	simulation.setMaximumCoefficientOfVariation(coefVar)
	simulation.setMaximumOuterSampling(outerSampling)
	#simulation.setBlockSize(blockSize)
