def evaluate(username, password, set_id, problem_id, x):
    ...
    ----------
        ...
        x : list, numpy.array
            Values of independent variables/random variables where the
    ...
    body = {
        ...
        "input_list": list(x),
    }  # obligés de caster x dans : list(x) ou np.array(x)

    # -----------------------------------------------
    # HTTP request
    # -----------------------------------------------
    r = requests.post(main_url + "evaluate", json=body, timeout=10)
