...
def run_FORM_simple(event, inputDistribution, nearestPointAlgo='AbdoRackwitz',
              NmaxIteration=100, eps=[1e-5]*4, physicalStartingPoint=None, seed=1234, verbose=False):
	...
	dfResult = dfResult.append(pd.DataFrame([result.getEventProbability()], index=["Probability of failure"]))
	dfResult = dfResult.append(pd.DataFrame([result.getGeneralisedReliabilityIndex()], index=["Generalised reliability index"]))
	dfResult = dfResult.append(pd.DataFrame([iter_number], index=["Number of iterations"]))
	dfResult = dfResult.append(pd.DataFrame([result.getStandardSpaceDesignPoint()], index=["Standard space design point"]))
	dfResult = dfResult.append(pd.DataFrame([result.getPhysicalSpaceDesignPoint()], index=["Physical space design point"]))
...
class FORMsystem():
    ...
    def getPhysicalSpaceDesignPoint(self):
        self._ustar = [self._FORMresult[i].getPhysicalSpaceDesignPoint() for i 
                     in range(self._nFORM)]
        return self._ustar
