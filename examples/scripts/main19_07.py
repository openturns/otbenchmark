...
from analytical_functions import gfun_22, gfun_8
from approximation_methods import run_FORM, FORMsystem
from simulation_methods import run_MonteCarlo, run_ImportanceSampling, run_LHS, run_SubSet
...
#~ #Run FORM
#~ FORM_result = run_FORM(event, myRandomVector, verbose=True, failure_domain=None)

#~ #Run CMC 
CMC_result = run_MonteCarlo(event, coefVar=0.01, outerSampling=3000, blockSize=100, verbose=True,\
							 failure_domain=None)
...
#~ #Run LHS 
#~ CMC_result = run_LHS(event, coefVar=0.01, outerSampling=3000, verbose=True, failure_domain=None)

#~ #Run SubSet 
#~ CMC_result = run_SubSet(event, coefVar=0.01, outerSampling=3000, verbose=True, failure_domain=None)

"""
TO DO LIST : 
phase 1 
- (ajouter SORM) 
- ajouter le strong maximum test avec une fonction paramétrique qui ressort "P* est unique" ou non 
